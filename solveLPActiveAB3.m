...
%                  report. Also solves primal directly if solveWeights fails.
...
%     beta = f*x2;
    ...
    kTic = tic;
    [a, rho, xi, fpval, exitflag] = solveWeights(beta, u, Hsel, D, 'exit', 0);
    % check linprog has given an acceptable solution
    if exitflag == -1
        [f, A, b, Aeq, beq, lb, ub] = createPrimalProblem(Hsel, D);
        [xP, ~ , exitflag, ~, ~] = linprog(f,A,b,Aeq,beq, ...
            lb, ub, [], options);
        if exitflag ~=1
            stophere = 1;
        end
...
%        if norm(a -a2) > 1e-5 
%         fprintf('diff vals %1.5f', norm(a-a2));
%         stophere = 1;
%        end
...
clear time
time.total = tt;
time.primal = sum(timePrimal);
time.kkt = sum(timeKKT);
time.dual = sum(timeDual);


%fprintf(1, repmat('\b',1, l_msg));
if verbose
    fprintf('R2MP iter = %d, cols_sel = %d, nonzero weight = %d, Primal: ', ...
            length(tt), length(P), sum(u>tol));
end
end
