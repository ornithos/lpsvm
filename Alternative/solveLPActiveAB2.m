...
%                  rather than overwriting variables. Probably useful if deploying.
%                  However, for ease I have not used this.
%
...
P2nsel = setdiff(1:N, P2);
Hnsel = H(:, ~P)';
%%%%%%%%%%%%%%%%%%
dbgon = 0;
if dbgon
    assignin('base','AB2P0',Hsel);
end
%%%%%%%%%%%%%%%%%%

tt = 1;
x = zeros(sum(P) + 1 ,1);    % + 1 for beta term

while(true),
    tic;
    
	try
		if any(sort(P2') ~= find(P))
			pause
			min(sort(P2') ~= find(P));
		end
	catch
...
%     beta = f*x2;
    ...
    if num_violations == 0 || sum(~P)==0 % No violating column: exit
    ...
    % elseif ass ==3    %% DEPRECATED SINCE NEEDS REWORK FOR LOGICAL PNSEL
%         searchMax = min(sum(Pnsel), numAddl*5);
...
%         newCols = [mvc; nextviolations(newCols)];
	...
	P2              = [P2 P2nsel(newCols)];
	P2nsel(newCols) = [];
    tradeIn         = createIndexVector(N, newCols, false, ~P);
    P               = P | tradeIn;
    Hsel            = H(:,P);
    Hnsel           = H(:, ~P)';
    ...
    %%%%%%%%%%%%%%%%%%
        if dbgon
            assignin('base',strcat('AB2P', num2str(tt)),Hsel);
        end
    %%%%%%%%%%%%%%%%%%

    % Warm start
    x = [x(1:(end-1)); zeros(sum(P) - (size(x,1)-1),1); x(end)];
    
    % admin
    NN = min(floor(NN),sum(~P));
    time(tt) = toc;
    tt = tt+1;
end
