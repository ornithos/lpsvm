function [u, beta, exitflag] = frankWolfe3(H, initial, D, maxIter, epsilon, ...
    tol, searchType, dbg)
...
for k = 1:maxIter
    ...
    lsFn = @(g) ((1-g).*currU + g.*u)*H'*currX';  %'Dual'
    lsFn = @(g) max(((1-g).*currU + g.*u)*H');    %'Primal'
    gamma = fminbnd(lsFn, 0, 1);
    currU = (1-gamma).*currU + gamma.*u;
