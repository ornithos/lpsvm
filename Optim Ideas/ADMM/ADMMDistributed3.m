function [primal, dual, fval, exitflag] = ADMMDistributed3(H, D, rho, eta, ...
...
for k = 1:maxIter
    ...
    for i = 1:npar
        ...
        [par{i}.x,~,ef] = quadprog(rho*par{i}.F, par{i}.f + rho*...
            [zeros(par{i}.n,1); (par{i}.u - zglobal)], par{i}.A, ...
    ...
    for i = 1:npar
        ...
        % update z / u
        par{i}.u    = par{i}.u +  par{i}.x((par{i}.n+1):end) - zglobal;
        ...
        par{i}.savX((par{i}.sX+2):(par{i}.sX+2+p),k) = zglobal;
        par{i}.savX((par{i}.sX+2+p+2):(par{i}.sX+2+2*p+2),k) = par{i}.u;
        
        d.X         = d.X + sum(par{i}.x((par{i}.n+1):end).^2);
        d.Z         = d.Z + sum(zglobal.^2);
        d.U         = d.U + sum(par{i}.u.^2);
        xmz         = (par{i}.x((par{i}.n+1):end) - zglobal);
    ...
    history.s_norm(k)  = rho*norm(zglobal - zOld);
