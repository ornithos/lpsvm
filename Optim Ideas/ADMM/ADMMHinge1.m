function [primal, dual, fval, exitflag] = ADMMHinge1(H, D, rho, alpha, ...
...
%
%   v1
%   -- simplicial projection enforced as regularisation
%
%   Arguments:
...
[p, n]         = size(H);
nu_inv         = D*n;
rho            = rho*n;      % obj scaled by n, so rho too.
% augment H with bias
M              = [ones(1,n); -H].*nu_inv;
MnormSq        = sum(M.*M);
...
for k = 1:maxIter
    ...
    % === x-update ====
    dotp        = sum(ZmUpC.*M,1);     % inner products <h, z-u>
    
    % -- calculate parameteric value t along line (-1,0) + t((0,h) - (-1,0))
    t = dotp./MnormSq;
    t = max(0, min(1,t));   % project onto line *segment*.
    
    % A + t(B-A).......       A = -c, B-A = M(:,i)
    X = bsxfun(@times, t, M);   % -c handled in next line (z - u) - (X - c)
    X = bsxfun(@plus, X, -c);
    ...
    % === z-update ====
    tmpz        = mean(Xhat+U,2);                      % GATHER (Consensus)
    z(2:end)    = projsplx(tmpz(2:end));          % SIMPLICIAL PROJECTION
    z(1)        = tmpz(1);
...
fval       = D*sum(primal.xi) - primal.rho;
