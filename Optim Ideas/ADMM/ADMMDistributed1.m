...
%   -- using iterative Linear System / Cholesky cache to solve QP.
...
% create arrays for f(x): Least Squares
    ...
    par{i}.H    = Hpar{i};
    par{i}.idx  = P{i};
    par{i}.f    = [D2*ones(np(i),1); zeros(np(i),1); -par{i}.n; 
                   par{i}.n; zeros(p,1)];   % note scaling by n and attribution
    par{i}.b    = [zeros(np(i),1); 1];
    par{i}.A    = [eye(np(i)), -eye(np(i)), -ones(np(i),1), ones(np(i),1), ...
                   par{i}.H'; zeros(1,2*np(i)+2), ones(1,p)];
    par{i}.sX   = 2*np(i)+p+2;
    par{i}.M    = [rho*eye(par{i}.sX), par{i}.A'; par{i}.A, ...
                   zeros(np(i)+1)];
    
    par{i}.chol = chol(par{i}.M*par{i}.M)';
    par{i}.bRHS = [(-rho*par{i}.f + [zeros(2*np(i)+2,1); ones(p,1)]);
                   -(D2+2*par{i}.n)*ones(np(i),1); 0];
    par{i}.z    = [zeros(2,1); zeros(par{i}.n*2, 1); ones(p,1)./p];
    par{i}.zOld = [zeros(2,1); zeros(par{i}.n*2, 1); ones(p,1)./p];
    par{i}.u    = zeros(par{i}.sX, 1);
end

saveX          = zeros(p+2+2*n+1,maxIter);
...
for k = 1:maxIter
    ...
    for i = 1:npar
        % Update x (this step
        par{i}.RHS = par{i}.bRHS + addlRHS(rho, par{i}.z, par{i}.u, ...
            par{i}.H, par{i}.n, p);
    ...
    zglobal    = zeros(p+2,1);
    for i = 1:npar
        zglobal = zglobal + par{i}.x((par{i}.n*2+1):end) ...
                    + par{i}.u((par{i}.n*2+1):end);       % GATHER z + u
    end
    zglobal    = max(zglobal./npar, 0);   % avg (z+u) / non-neg projection
    ...
    for i = 1:npar
        ...
        % update z / u
        xlocal      = par{i}.x(1:(par{i}.n*2));
        par{i}.z    = [max(xlocal + par{i}.u(1:(par{i}.n*2)), 0);
                       zglobal];  % append global update
        par{i}.u    = par{i}.u + par{i}.x - par{i}.z;
        
        % >>>> for diagnostics
        saveX((p+2)+P{i},k) = par{i}.z(1:par{i}.n);
        saveX((p+2)+n+P{i},k) = par{i}.z((par{i}.n+1):(2*par{i}.n));
...
%% Output
saveX(p+2*n+3,1:k) = history.objval;
fprintf('num iter: %d, fpval: %1.7f, rho: %1.7f\n', k, ...
    history.objval(k), zglobal(2) -zglobal(1));
