function [primal, dual, fval, exitflag] = ADMMDistributed7(H, D, rho, alpha, ...
...
%   v7
%   -- Over-relaxation.
%   -- Adaptive rho. ** did not prove effective - reduced convergence when
%      a more oscillatory path. ** (DEPRECATED)
...
% adaptive parameter parameters (!)
adaMu         = 10;     % Relative gap required before adjustment.
adaTau        = 1;     % (1+tau) = multiplier of rho when gap exceeded.
saveRho       = zeros(maxIter,1);  % Record of movement.
...
%     saveRho(k) = rho;
%     updatePars = 0;
%     adaRatio = (history.r_norm(k)/history.s_norm(k))*...
%             (history.eps_dual(k)/history.eps_pri(k));
%     
%     if adaRatio > adaMu
%         updatePars = 1.0/(1+adaTau/k);
%         rho = rho./updatePars;
%         
%     elseif adaRatio < 1/adaMu
%         updatePars = 1.0+adaTau/k;
%         rho = rho./updatePars; 
%     end
%     if updatePars ~= 0    % update all pars that are dependent on rho
%         gl_U = gl_U.*updatePars;
%         for i = 1:npar
%             par{i}.M = [rho*eye(par{i}.sX), par{i}.A'; par{i}.A, zeros(np(i)+1)];
%             par{i}.u = par{i}.u.*updatePars;
%         end
%     end
end
